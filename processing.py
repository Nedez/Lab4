...
def get_adjs(doc):
    result = [
        token.text.lower() for token in doc if token.pos_ == "ADJ"
    ]
    return result
...
test_doc = nlp(data["review"][2])
print(get_adjs(test_doc))
print(get_ents(test_doc))
print(get_number_of_sents(test_doc))
...
for review, sentiment in tqdm(data.to_records(index=False)):
    ...
    for adj in adjs:
        if sentiment == 'positive':
            pos_adj_counter[adj]+=1
        else:
            neg_adj_counter[adj]+=1
